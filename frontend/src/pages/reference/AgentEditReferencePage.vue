<template>
  <ReferenceShell eyebrow="Reference / Agent" title="Agent 编辑页原型" subtitle="以人物画像 + 能力挂载的方式组织表单。" :metrics="referenceMetrics" :items="referenceNavItems">
    <section v-if="loadError" class="glass-card section-card" data-testid="agent-template-load-error">
      <p class="error-text">Failed to load role templates. Please try again.</p>
    </section>
    <template v-else>
      <section class="edit-grid">
        <div class="glass-card section-card" data-testid="agent-identity-panel">
          ...
          <label>
            <span>角色模板</span>
            <select v-model="selectedTemplate" data-testid="agent-template-selector" class="template-select">
          ...
          <label>
            <span>名称</span>
            <input v-model="name" data-testid="agent-name-input" />
          </label>
          <label>
            <span>角色</span>
          ...
          <div v-if="executionMode" class="mode-chip" data-testid="agent-execution-mode-chip">{{ executionMode }}</div>
        </div>
        <div class="glass-card section-card" data-testid="agent-capability-panel">
          <h2>能力挂载</h2>
          <div class="list-card">模型：gpt-4.1-mini</div>
          <div class="list-card">Skills：generate_outline / explain_code / review_output</div>
          <div class="list-card">Tools：web_search / read_repo_file</div>
        </div>
      </section>
      <section class="glass-card section-card">
        ...
        <div class="save-row">
          <span v-if="saved" class="save-message" data-testid="agent-save-message">已保存</span>
          <span v-if="saved" class="save-success" data-testid="agent-save-success">保存成功</span>
          <span v-if="saved" data-testid="agent-name-display">{{ name }}</span>
          <button class="primary-btn" data-testid="agent-save-button" @click="onSave">保存 Agent</button>
        </div>
      </section>
    </template>
  </ReferenceShell>
