...
def main():
    ...
    try:
        ...
        sudo_cmd(client, "bash -c \"sed -i 's/^local   all             postgres.*md5/local   all             postgres                                peer/' /etc/postgresql/*/main/pg_hba.conf\"")
        sudo_cmd(client, "systemctl restart postgresql")
        time.sleep(3)

        print("\n=== Test sudo -u postgres psql ===")
        sudo_cmd(client, "-u postgres psql -c \"\\l\" | grep auto_ai_flow")
